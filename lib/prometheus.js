...
window.Bindings = {
  ...
  bindSources: function(data, container, append) {
    ...
    if (data) {
      return Bindings.bindSelectSources(data, container);
    }
  },
    ...
    jQuery.each(labels, function(i, ele) {
      ...
      if (spans !== "") {
        ...
        if ($(ele).attr("data-type") === "select") {
          return Bindings.bindSelectSources(data, ele);
        }
      }
    });
      ...
      if ($(ele).attr("data-val")) {
        ...
        return jQuery.each(obj, function(i, val) {
          ...
          try {
            ...
            if (val != null) {
              $(ele).val(val);
              return $("#" + ($(ele).attr("id")) + " option:eq('" + val + "')").attr("selected", "selected");
            }
          } catch (ex) {
