    ...
    return jQuery.each(lists, function(i, ele) {
      ...
      if ($(ele).attr("data-val")) {
        return jQuery.each(data, function(i, obj) {
          var ad_name, additional_hidden, additional_property, adtl, blur, change, classes, data_options, input, label, li, name, property, _ref, _ref2;
          property = $(ele).attr("data-val").split(".");
          additional_property = [];
          try {
          ...
          try {
            try {
              for (i = 0, _ref = additional_property.length - 1; 0 <= _ref ? i <= _ref : i >= _ref; 0 <= _ref ? i++ : i--) {
